...
(function ($) {
    ...
    var TeamBlock =
        ...
        function () {
            function TeamBlock(source, // Where base of the information propagated from
                ...
                this.order = order;
                this.seed = seed;
                this.score = score; // A pair of teams is created simultaneously for a match so the sibling
    ...
    function postProcess(container, w, f) {
        ...
        container.find(".team").mouseover(function () {
            var teamId = $(this).attr("data-teamid"); // Don't highlight BYEs
    ...
    var endOfBranch = function endOfBranch() {
        throw new EndOfBranchException();
    };

    var winnerMatchSources = function winnerMatchSources(teams, m) {
        return function () {
    ...
    var winnerAlignment = function winnerAlignment(match, skipConsolationRound) {
        return function (tC) {
            tC.css("top", "");
    ...
    var loserMatchSources = function loserMatchSources(winners, losers, matchCount, m, n, r) {
        return function () {
    ...
    var loserAlignment = function loserAlignment(teamCon, match) {
        return function () {
            return teamCon.css("top", match.el.height() / 2 - teamCon.height() / 2 + "px");
        };
    };

    var mkMatchConnector = function mkMatchConnector(centerConnectors) {
    ...
    function prepareFinals(finals, winners, losers, opts, topCon, resizeContainer) {
        ...
        var finalMatch = round.addMatch(function () {
            ...
            if (!opts.skipSecondaryFinal && !match.winner().name.isEmpty() && match.winner().name === losers.winner().name) {
                ...
                var doRenderCb = function doRenderCb() {
    ...
    var Round =
        /** @class */
        function () {
            function Round(bracket, previousRound, roundNumber, // TODO: results should be enforced to be correct by now
            ...
            Round.prototype.addMatch = function (teamCb, renderCb) {
                ...
                var teamA = function teamA() {
                    return teams[0].source();
                };

                var teamB = function teamB() {
    ...
    var Bracket =
        ...
        function () {
            ...
            Bracket.prototype.addRound = function (doRenderCb) {
                ...
                var previous = id > 0 ? Option.of(this.rounds[id - 1]) : Option.empty(); // Rounds may be undefined if init score array does not match number of teams

                var roundResults = this.initResults.map(function (r) {
                    return r[id] === undefined ? new ResultObject(Score.empty(), Score.empty(), undefined) : r[id];
                });
    ...
    function mkConnector(roundMargin, connector, teamCon, align) {
        ...
        var _a = calculateHeight(connector.height),
            height = _a.height,
            drop = _a.drop;
        ...
        src.css(align, -width - 2 + "px"); // Subtract 1 due to line thickness and alignment mismatch caused by
    ...
    function teamElement(roundNumber, match, team, opponent, isReady, isFirstBracket, opts, resultId, topCon, renderAll) {
        ...
        tEl.append(sEl); // Only first round of BYEs can be edited
    ...
    var Match =
        ...
        function () {
            ...
            Match.prototype.render = function () {
                ...
                this.matchCon.empty();
                this.teamCon.empty(); // This shouldn't be done at render-time
    ...
    var undefinedToNull = function undefinedToNull(value) {
        return value === undefined ? null : value;
    };

    var wrapResults = function wrapResults(initResults) {
    ...
    var JqueryBracket = function JqueryBracket(opts) {
        var resultId = new ResultId();
        var data = opts.init;
        var isSingleElimination = data.results.length <= 1; // 45 === team height x2 + 1px margin
        ...
        var mkMatch = function mkMatch(round, match, seed, results, renderCb, isFirstBracket, options) {
    ...
    var assertNumber = function assertNumber(opts, field) {
        if (opts.hasOwnProperty(field)) {
            var expectedType = "number";

            var type = _typeof(opts[field]);
    ...
    var assertBoolean = function assertBoolean(opts, field) {
    ...
    var assertGt = function assertGt(expected, opts, field) {
        var value = opts[field];
    ...
    var isPow2 = function isPow2(x) {
        return x & x - 1;
    };

    var methods = {
        init: function init(originalOpts) {
            ...
            var disableToolbarType = _typeof(opts.disableToolbar);
            ...
            var disableTeamEditType = _typeof(opts.disableTeamEdit);
